...
var _a, _b, _c, _d;
if (status) {
  const config = JSON.parse(fse.readFileSync(configUrl));
...
var MixinRender = class {
    ...
    importGlobal.forEach((item) => {
      item.specifiers.forEach((specifier) => {
        ...
        if (this.mixinList.includes(name)) {
          ...
          const fileStatus = fse3.existsSync(file);
          const indexFileStatus = fse3.existsSync(indexFile);
  ...
  async initMixin() {
    const fileCallback = async (item) => {
      const filePath = path2.resolve(item.path);
      const fileCode = fse3.readFileSync(filePath, { encoding: "utf-8" });
...
var ComponentsRender = class {
  ...
  loopLookPath(filePath, key) {
    const fileContent = fse4.readFileSync(filePath, "utf8");
...
var PinnaNode = class {
  ...
  async renderPinia() {
    const bodyCode = await generate2.default(this.astNode);
  ...
  async initPinia() {
    const fileCallback = async (key) => {
      ...
      if (!PinnaNode.cacheNode.has(piniaModuleItem2 == null ? void 0 : piniaModuleItem2.importName)) {
        ...
        const status2 = fse5.existsSync(filePath);
        if (status2) {
          const fileCode = await fse5.readFileSync(path4.resolve(filePath), { encoding: "utf-8" });
...
var scriptRender = async (code, options2, filePath) => {
  ...
  const render3 = async () => {
    const newCode = await generate3.default(newAst).code;
    return {
      newCode,
      newAst
    };
...
var templateRender = async (dom, scriptData, filePath, options2) => {
  ...
  const loopInterpolation = (elem) => {
    ...
    RenderCallbacks.push(async () => {
      const callback = async (item) => {
        const value = await generate4.default(item.ast);
  ...
  const replaceAttribsVal = (attribs, key) => {
    ...
    RenderCallbacks.push(async () => {
      const attribsCode = await generate4.default(ast);
...
var getAllDirByFilename = (dir, compileDir) => {
  const dirPath = path5.resolve(__dirname, dir);
