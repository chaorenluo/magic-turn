...
var PinnaNode = class {
  ...
  async renderPinia() {
    let bodyCode = await import_generator2.default.default(this.astNode);
...
var templateRender = async (dom, scriptData, filePath, options2) => {
  ...
  const loopInterpolation = (elem) => {
    ...
    RenderCallbacks.push(async () => {
      const callback = async (item) => {
        let value = await import_generator4.default.default(item.ast);
  ...
  const replaceAttribsVal = (attribs, key) => {
    ...
    RenderCallbacks.push(async () => {
      let attribsCode = await import_generator4.default.default(ast);
