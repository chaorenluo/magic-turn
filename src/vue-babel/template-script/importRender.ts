...
const ImportRender = (newAst:t.File, options:Config) => {
  return {
    ...
    addImportGlobal (node:any) {
      ...
      this.importGlobal.push(node as never)
    },
    ...
    renderImportGlobal () {
      if (this.importGlobal.length > 0) {
        this.importGlobal.forEach((item:any) => {
    ...
    renderRef () {
      if (this.refKey.size > 0) {
        this.refKey.forEach(refName => {
          ...
          const refNode = createFnVariable(refName, 'ref', [t.nullLiteral()])
