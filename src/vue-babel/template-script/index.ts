...
const scriptRender = async (code: string, options:Config, filePath) => {
  const newAst = parse('', {
    sourceType: 'module',
    plugins: ['jsx']
  })
  ...
  const componentsRender:ComponentsRender = new ComponentsRender()
  const importRender = ImportRender(newAst, options)
  const lifeCycleRender: LifeCycleRender = new LifeCycleRender(options, newAst, importRender)
  ...
  const isExportIdentifier = () => {
    ...
    if (exportObj) {
      ...
      const file = t.file(t.program(astArr))
      ast = file
    }
  }
    ...
    MemberExpression (path) {
      if (path.node.object.type === 'ThisExpression' || path.node.object.name === '_this') {
        ...
        if (name && name.indexOf('$') > -1) {
          ...
          if (name === '$el') {
            importRender.addVueApi('ref')
            const el = t.memberExpression(t.identifier('el_ref'), t.identifier('value'))
  ...
  const render = async () => {
    const newCode = await generate.default(newAst).code
    return {
      newCode,
      newAst
    }
